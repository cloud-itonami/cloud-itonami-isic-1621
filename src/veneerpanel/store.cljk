(ns veneerpanel.store
  "SSoT for the wood-panel plant-operations coordination actor, behind
  ...
  veneer/panel capability library, no jurisdiction-scoped
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             panel/veneer-grade/volume/output-quality
                             ...
                             `:shipped-volume-cubic-m` tracks the
                             batch's own cumulative-shipped ground
                             truth.
    - `equipment`         -- a veneer-lathe/hot-press/glue-spreader
    ...
    - `maintenance`       -- a scheduled veneer-peeling/pressing/
                             gluing-line maintenance-window DRAFT
                             against a piece of equipment
                             (`veneerpanel.registry`'s
    ...
    - `shipments`         -- a proposed outbound-panel-shipment DRAFT
                             (`veneerpanel.registry`'s
                             `register-shipment`).
  ...
  independently-recomputed volume, approved by whom, which safety
  concern was flagged' is always a query over an immutable log -- the
  audit trail a mill owner or panel buyer trusting this coordinator
  needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (veneerpanel.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (veneerpanel.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :grade :veneer-a :panel-type :plywood
                :thickness-mm 12.0 :volume-cubic-m 500.0 :output-quality-percent 96.0
                :verified? true :registered? true
                :shipped-volume-cubic-m 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :grade :structural-i :panel-type :osb
                :thickness-mm 18.0 :volume-cubic-m 80.0 :output-quality-percent 92.0
                :verified? true :registered? true
                :shipped-volume-cubic-m 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :grade :mdf-standard :panel-type :mdf
                :thickness-mm 16.0 :volume-cubic-m 600.0 :output-quality-percent 89.0
                :verified? false :registered? false
                :shipped-volume-cubic-m 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :hot-press
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :glue-spreader
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `veneerpanel.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `veneerpanel.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
      ...
      (let [shipment-id (first path)
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-volume-cubic-m]
                                  (fn [prev]
                                    (+ (double (or prev 0.0))
                                       (double (or (:volume-cubic-m value) 0.0))))))))
        result)
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered hot-press unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  glue-spreader unit (blocks any maintenance scheduling against it) --
