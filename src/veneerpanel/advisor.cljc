(ns veneerpanel.advisor
  "VeneerPanelAdvisor -- the *contained intelligence node* for the
  wood-panel plant-operations coordination actor.

  It normalizes production-batch patches (panel/veneer-grade/volume/
  output-quality), drafts a veneer-lathe/hot-press/glue-spreader
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound-panel-shipment
  ...
  a real veneer-peeling/pressing/gluing-line-equipment actuation,
  press-cycle execution, or freight dispatch. Every output is censored
  downstream by `veneerpanel.governor` before anything touches the
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `veneerpanel.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct veneer-
                                 ; peeling/pressing/gluing-line-
                                 ; equipment-control or press-cycle-
                                 ; finalize effect
  ...
  contract every caller of this actor agrees to) -- `veneerpanel.governor`
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [veneerpanel.registry :as registry]
            [veneerpanel.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's grade, volume,
  output-quality or verification status. High confidence, low
...
(defn- schedule-maintenance
  "Draft a veneer-lathe/hot-press/glue-spreader maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `veneerpanel.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft an equipment-safety/formaldehyde-emission/fire-hazard
  ...
  administrative technicality). See `veneerpanel.phase`: no phase ever
  adds this op to a phase's `:auto` set; `veneerpanel.governor` also
...
(defn- coordinate-shipment
  "Draft an outbound-panel-shipment coordination proposal against a
  production batch. The advisor passes through the caller's own
  claimed volume -- it does NOT invent one, and `veneerpanel.governor`
  ...
  (let [batch-id (:batch-id value)
        ...
        over-volume? (and b (registry/shipment-volume-exceeded?
                             b (:volume-cubic-m value)))]
    {:summary    (str subject " 向け出荷調整提案 ("
                      (:volume-cubic-m value) " m³)"
...
(def ^:private system-prompt
  (str "あなたは木質パネル工場プラント運用コーディネーターの助言者です。"
       ...
       "プレスサイクルの確定(finalize)や単板旋盤・プレス・接着ライン等の設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `veneerpanel.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :veneerpanel-advisor-proposal
   :op         (:op request)
