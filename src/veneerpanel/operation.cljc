(ns veneerpanel.operation
  "VeneerPanelOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (VeneerPanelAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Wood Panel
  ...
  (:require [langgraph.graph :as g]
            ...
            [veneerpanel.advisor :as advisor]
            [veneerpanel.governor :as governor]
            [veneerpanel.phase :as phase]
...
(defn build
  "Compiles a VeneerPanelOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `veneerpanel.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; VeneerPanelAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Wood Panel Plant Operations Governor -- independent censor.
      (g/add-node :govern
