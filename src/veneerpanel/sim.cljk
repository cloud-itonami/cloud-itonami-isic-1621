(ns veneerpanel.sim
  ...
  production volume, a proposal that tries to FINALIZE a press-cycle
  run (permanently blocked, no override), a double-schedule of the
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on equip-001 (verified, registered, hot-press -- escalates, approve) ==")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:grade :veneer-a}}
                       coordinator))

    (println "== unrecognized op -> HARD hold ==")
    (println (exec-op actor "t6"
                       {:op :actuate-hot-press :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on equip-002 (UNVERIFIED/unregistered glue-spreader -> HARD hold) ==")
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (10 m³ would exceed volume 80 vs shipped 75 -> HARD hold) ==")
