(ns veneerpanel.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control veneer-peeling/pressing/gluing-
  line equipment directly... does NOT authorize or execute
  maintenance/press operations') implemented faithfully. The single
  invariant under test:

    VeneerPanelAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Wood Panel Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:grade :veneer-a}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :veneer-a (:grade (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-hot-press :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest press-finalize-is-held-and-permanently-blocked
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t8"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-3"
                     :value {:equipment-id "equip-001" :maintenance-type :press-run-finalize
                             :scheduled-date "2026-09-01" :finalize? true}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:press-finalize-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "equip-001" :severity :moderate
                                            :description "formaldehyde emission near press"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
